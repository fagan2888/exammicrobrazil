...
'''
This code generates plots for the effect of changing delta on equilibirum values 
'''
...
mp.plot(grid_delta, effective, 'k-')
mp.grid()
mp.title('"Effective" wage in the formal sector ($(1 - (1 - \\delta s_F)\\tau_\\omega)w_F$) as function of $\\delta$')
mp.xlabel('$\\delta$')
mp.ylabel('$(1 - (1 - \\delta s_F)\\tau_\\omega)w_F$')
mp.savefig('effective.pdf')
mp.show()